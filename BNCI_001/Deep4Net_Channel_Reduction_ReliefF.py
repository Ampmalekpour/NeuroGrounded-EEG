...
"""
Deep4Net on BNCI2014_001 with fold-wise ReliefF-based channel reduction.
...
- Model/training:
    ...
    * TTA evaluation
- Channel reduction:
    * ReliefF computed ONLY on the training fold
    * top-K channels selected per fold
    * train/test rebuilt using only those channels
"""
...
OUT_ROOT = "Deep4Net_BNCI2014_001_4Fold_ReliefF"
...
# =========================================================
# 4) ReliefF channel selection
# =========================================================
def compute_relieff_importance(X, y, n_neighbors=10):
    """
    ReliefF-style channel importance using trial-wise channel variance.
    ...
    # Feature per trial/channel: variance over time
    features = np.var(X, axis=2).astype(np.float64)  # [N, C]

    # Min-max normalize each channel feature
    ...
    for i in range(n_trials):
        ...
        # Near hits: same class
        hit_idx = np.where(y == y[i])[0]
        hit_idx = hit_idx[hit_idx != i]
        if len(hit_idx) == 0:
            continue

        n_hit = min(n_neighbors, len(hit_idx))
        near_hits = hit_idx[np.argsort(dists[hit_idx])[:n_hit]]

        diff_hits = np.mean(
            np.abs(features[i] - features[near_hits]),
            axis=0
        )

        # Near misses: different class
        miss_idx = np.where(y != y[i])[0]
        if len(miss_idx) == 0:
            continue

        n_miss = min(n_neighbors, len(miss_idx))
        near_misses = miss_idx[np.argsort(dists[miss_idx])[:n_miss]]

        diff_misses = np.mean(
            np.abs(features[i] - features[near_misses]),
            axis=0
        )

        weights += diff_misses - diff_hits

    weights = weights / max(n_trials, 1)
    return normalize_importance(weights)


def select_top_channels_relieff(X_train_full, y_train, full_channels, k):
    importance = compute_relieff_importance(
        X_train_full,
        y_train,
...
def train_one_fold(
    ...
    # Deep4Net expects fixed n_times=1000 in this setup
    if X_train.shape[2] != N_SAMPLES_RECEPTIVE:
        X_train = X_train[:, :, -N_SAMPLES_RECEPTIVE:]
    if X_test.shape[2] != N_SAMPLES_RECEPTIVE:
        X_test = X_test[:, :, -N_SAMPLES_RECEPTIVE:]
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        test_acc_base = evaluate_tta(model, test_loader)

        if epoch >= SWA_START:
            test_acc_swa = evaluate_tta(swa_model, test_loader)
            if test_acc_swa >= test_acc_base:
                eval_acc = test_acc_swa
                source = "swa"
                chosen_model = swa_model.module
            else:
                eval_acc = test_acc_base
                source = "base"
                chosen_model = model
        else:
            test_acc_swa = float("nan")
            eval_acc = test_acc_base
            source = "base"
            chosen_model = model
        ...
        if eval_acc > best_acc:
            best_acc = eval_acc
            best_source = source
            best_state = copy.deepcopy(chosen_model.state_dict())
        ...
        if epoch % 25 == 0 or epoch == 1 or epoch == EPOCHS:
            ...
            print(
                ...
                f"test_base={test_acc_base*100:.2f}% | "
                f"test_swa={test_acc_swa*100 if epoch >= SWA_START else float('nan'):.2f}% | "
                f"best={best_acc*100:.2f}% ({best_source})"
            )

    if EPOCHS >= SWA_START:
        update_bn(swa_bn_loader, swa_model, device=DEVICE)

    final_model = build_deep4net_model(n_chans=X_train.shape[1], n_classes=4)
    ...
    del model, optimizer, scheduler, swa_model, final_model
    gc.collect()
    if torch.cuda.is_available():
...
def run_subject(subject_id):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_full, y), start=1):
        ...
        # ReliefF selection on training fold only
        selected_channels, importance = select_top_channels_relieff(
        ...
        np.save(os.path.join(fold_dir, "relieff_importance.npy"), importance)
...
def main():
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("Deep4Net + 4-fold CV + ReliefF-based channel reduction\n\n")
        for r in all_results:
            f.write(
                f"Subject {r['subject_id']:02d} | "
                f"mean={r['mean_acc']*100:.2f}% | "
