...
"""
Deep4Net on BNCI2014_001 with fold-wise Mutual-Information-based channel reduction.
...
- Model/training:
    ...
    * TTA evaluation
- Channel reduction:
    * Mutual Information computed ONLY on the training fold
...
OUT_ROOT = "Deep4Net_BNCI2014_001_4Fold_MI"
...
def compute_mi_importance(X, y):
    ...
    Fast EEG-style MI channel ranking:
    - For each trial and each channel, compute variance over time
    - Use those channel-wise variance features to estimate MI with labels

    Returns normalized channel importance of shape [C].
    """
    features = np.var(X, axis=2)  # [N, C]
    ...
    return normalize_importance(mi)


def select_top_channels_mi(X_train_full, y_train, full_channels, k):
    importance = compute_mi_importance(X_train_full, y_train)
...
def train_one_fold(
    ...
    # Deep4Net here expects fixed n_times = 1000 in this setup
    # so crop epoch window [-0.5, 4.0) -> last 1000 samples
...
def run_subject(subject_id):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_full, y), start=1):
        ...
        # MI selection on training fold only
        selected_channels, importance = select_top_channels_mi(
        ...
        np.save(os.path.join(fold_dir, "mi_importance.npy"), importance)
...
def main():
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("Deep4Net + 4-fold CV + MI-based channel reduction\n\n")
        for r in all_results:
            f.write(
                f"Subject {r['subject_id']:02d} | "
                f"mean={r['mean_acc']*100:.2f}% | "
