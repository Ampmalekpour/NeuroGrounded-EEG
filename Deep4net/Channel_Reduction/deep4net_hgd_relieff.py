...
OUTPUT_DIR = "Deep4Net_HGD_ReliefF_OnlyReduced"
...
# ReliefF
# =========================================================
def compute_relieff_scores(X, y, n_neighbors=10):
    """
    X shape: [n_trials, n_channels, n_times]
    ReliefF is applied to channel-level features:
        feature(channel) = variance over time per trial
    """
    n_trials, n_channels, _ = X.shape

    feats = np.var(X, axis=2)  # [n_trials, n_channels]

    fmin = feats.min(axis=0, keepdims=True)
    fmax = feats.max(axis=0, keepdims=True)
    feats = (feats - fmin) / (fmax - fmin + 1e-8)

    weights = np.zeros(n_channels, dtype=np.float64)

    for i in range(n_trials):
        dists = np.linalg.norm(feats - feats[i], axis=1)
        dists[i] = np.inf
        ...
        near_hits = same[np.argsort(dists[same])[:n_neighbors]]
        near_miss = diff[np.argsort(dists[diff])[:n_neighbors]]

        if len(near_hits) == 0 or len(near_miss) == 0:
            continue

        hit_term = np.mean(np.abs(feats[i] - feats[near_hits]), axis=0)
        miss_term = np.mean(np.abs(feats[i] - feats[near_miss]), axis=0)

        weights += (miss_term - hit_term)

    weights /= max(n_trials, 1)
    return normalize_scores(weights)


def select_channels_via_relieff(subject_id, n_best):
    X_rank, y_rank = load_subject_data(subject_id, CANDIDATE_CHANNELS)
    scores = compute_relieff_scores(X_rank, y_rank, n_neighbors=RELIEFF_NEIGHBORS)
...
def run_subject(subject_id):
    ...
    selected_channels, scores, ranked = select_channels_via_relieff(
        subject_id,
        N_BEST_CHANNELS
    )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("Deep4Net + ReliefF channel selection + reduced-channel training only")
    print("=" * 80)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Deep4Net + ReliefF reduced-channel experiment\n")
        f.write("=" * 80 + "\n\n")
